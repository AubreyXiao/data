...
all_sentences = train_tokens + unsup_tokens + test_tokens + tokens1 + tokens2
...
context = 15 #contect window size
downsampling = 1e-3
print("Training models!")

model = word2vec.Word2Vec(all_sentences,workers=num_workers,size=num_features,min_count=min_word_count,window=context,sample=downsampling,negative=15)

#init_sims wiil make the model memory efficient
model.init_sims(replace=True)

#save the modle
filename = "300features_10min_15window_NEG15_extra"
model.save(filename)

#save the modle in text form
filename1 = '300features_10min_15window_NEG15_extra.txt'
